...
def creer_features_irve(df_irve, col_geo="code_geo_total"):
    ...
    df_final = df_irve.groupby(col_geo).agg(
        ...
        part_charge_rapide=('borne_rapide', 'mean')
    )

    top_operateur = (df_irve.groupby(col_geo)['nom_operateur'].apply(lambda x: x.mode().iloc[0] if not x.empty else "inconnu").to_frame(name='top_operateur'))
    df_final = df_final.join(top_operateur)

    env = pd.get_dummies(df_irve[[col_geo, 'implantation_station_clean']], columns=['implantation_station_clean'], prefix='', prefix_sep='')
